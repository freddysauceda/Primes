...
def newHeuristic(start, end):
    # Function to check the hamming distance of two numbers for the heuristic
    # Returns maxint if unsolvable, hamming distance if otherwise
    # Edited version also takes into account the number of reachable primes
    # and their hamming distance to the target value and adds the smallest
    # one to the count

    # First find original Hamming Distance
    count = hammingDistance(start, end)

    # Min is infinity to get the right minimum value
    min = sys.maxint

    # For every possible neighbor, find the minimum Hamming Distance to the end
    for neighbor in getPossibleActions(start):
        neighbor_ham = hammingDistance(neighbor, end)
        if neighbor_ham < min:
            min = neighbor_ham

    # Add the minimum to the count
    count += min

    # Return the edited Hamming Distance
    return count
    ...
    while not queue.empty():
        ...
        for next in getPossibleActions(current):
            ...
            if next not in cost_from or new_cost < cost_from[next]:
                ...
                priority = new_cost + newHeuristic(finalPrime, next)
